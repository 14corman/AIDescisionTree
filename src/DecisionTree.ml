...
module DTree = struct
  ...
  module DotInput = 
    Dot.Parse
      (B)
      (struct 
        let get_label attr_list = List.fold_left (fun a attr -> 
  ...
  let string_of_string_map map = (Feature_map.fold (fun key value string -> string ^ value ^ ",") map "[") ^ "]"
  let str_of_str_map_w_key map = (Feature_map.fold (fun key value string -> string ^ key ^ ":" ^ value ^ ",") map "[") ^ "]"
  let string_of_int_map map = (Feature_map.fold (fun key value string -> string ^ string_of_int value ^ ",") map "[") ^ "]"
  ...
  let build_tree x_map y attr_map =
    ...
    let compute_num_samples (list : int list) = 
      List.fold_left (fun map label -> if (Feature_map.mem (string_of_int label) map) then 
                         ...
                         Feature_map.add (string_of_int label) 0 map) Feature_map.empty num_classes in
