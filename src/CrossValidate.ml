...
*)
let rec shuffle features (feature_map : int list Feature_map.t) (labels : int list) =
  let rec shuffler k feature_m labels =
    if k < 1 then
      ...
      let feat_map = Feature_map.map (fun val_list -> let (tail, head) = List.fold_right2 (fun v b (h, t) -> if b then (v :: h, t) else (h, v :: t)) val_list parts ([],[]) in
...
let shuffle_and_partition (k : int) (features : string list) (feature_map : int list Feature_map.t) (labels : int list) = 
...
let cross_validate (k : int) (d : depth) (data_file : string) =
  ...
  let features = Feature_map.fold (fun feat l so_far -> feat :: so_far) feature_map [] in
